package com.example.tp.formes;

import java.util.ArrayList;
import java.util.Arrays;

public abstract class PileTab<T extends Comparable<T>> extends PileA<T> {
    protected T[] formes;

    public T[] getFormes() {
        return formes;
    }

    protected int sommet;

    @SuppressWarnings("unchecked")
    protected PileTab(int taille) {
        formes = (T[]) new ArrayList<>(taille).toArray();
        sommet = 0;
    }

    protected PileTab(T[] formes, int taille) {
        this.formes = Arrays.copyOf(formes, taille > formes.length ? taille : formes.length);
        sommet = taille;
    }

    protected PileTab(PileTab<T> formes) {
        this(formes.getFormes(), formes.taille());
    }

    protected void ajouterComp(T f) {
        if (!vide()) {
            var s2 = sommet();
            if (s2.compareTo(f) <= 0) {
                empiler(f);
            } else {
                depiler();
                ajouterComp(f);
                empiler(s2);
            }
        } else {
            empiler(f);
        }
    }

    @Override
    public void depiler() {
        if (vide())
            throw new IndexOutOfBoundsException();
        formes[--sommet] = null;
    }

    @Override
    public T sommet() {
        if (vide())
            throw new IndexOutOfBoundsException();
        return formes[sommet - 1];
    }

    @Override
    public boolean vide() {
        return sommet == 0;
    }

    @Override
    public int taille() {
        return sommet;
    }

    @Override
    public int hashCode() {
        final int prime = 31;
        int result = 1;
        result = prime * result + Arrays.hashCode(formes);
        result = prime * result + sommet;
        return result;
    }

    @Override
    public boolean equals(Object obj) {
        return super.equals(obj);
    }

}